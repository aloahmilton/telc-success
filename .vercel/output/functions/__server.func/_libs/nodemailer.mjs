...
//#endregion
//#region node_modules/nodemailer/lib/shared/objects.js
var require_objects = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	/**
	...
	*/
	module.exports.copyOwnKeys = (target, source, skip) => {
		Object.keys(source || {}).forEach((key) => {
			if (module.exports.isProtoKey(key) || skip && skip(key)) return;
	...
	module.exports.assign = function() {
		...
		args.forEach((source) => {
			Object.keys(source || {}).forEach((key) => {
				...
				if (["tls", "auth"].includes(key) && source[key] && typeof source[key] === "object") target[key] = module.exports.copyOwnKeys(target[key] || {}, source[key]);
	...
	*/
	function _handleAddress(tokens, depth) {
		...
		if (isGroup) {
			...
			if (addressFromQuotedText && address.address) address.address = _quoteLocalPart(address.address);
...
var require_mime_node = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	var DOT_ATOM = /* @__PURE__ */ new RegExp("^[A-Za-z0-9!#$%&'*+\\-/=?^_`{|}~\\x80-\\uFFFF]+(?:\\.[A-Za-z0-9!#$%&'*+\\-/=?^_`{|}~\\x80-\\uFFFF]+)*$");
	...
	module.exports = class MimeNode {
		...
		*/
		setEnvelope(envelope) {
			...
			[
				...
				if (envelope[key]) this._convertAddresses(this._parseEnvelopeAddresses(envelope[key]), this._envelope.to);
			});
		...
		_parseAddresses(addresses) {
			return [].concat.apply([], [].concat(addresses).map((address) => {
				if (address && address.address) {
					...
					if (normalized === address.address && typeof address.name === "string") return [address];
		...
		*/
		_encodeAddressName(name) {
			if (!/^[\w ]*$/.test(name)) if (/^[\x20-\x7e]*$/.test(name)) return mimeFuncs.quoteString(name);
	...
	var MailComposer = class {
		...
		*/
		_getIcalEvent() {
			if (!this._icalEvent) {
				let icalEvent;
				if (typeof this.mail.icalEvent === "object" && (this.mail.icalEvent.content || this.mail.icalEvent.path || this.mail.icalEvent.href || this.mail.icalEvent.raw)) icalEvent = copyOwnKeys({}, this.mail.icalEvent);
		...
		resolveAll(callback) {
			...
			const resolveNext = () => {
				...
				shared.resolveContent(...args, {
					...
					if (args[0][args[1]] && typeof args[0][args[1]] === "object" && !Buffer.isBuffer(args[0][args[1]])) shared.copyOwnKeys(node, args[0][args[1]], (key) => key in node || [
						"content",
						"path",
						"href",
						"raw"
		...
		normalize(callback) {
			...
			this.resolveAll((err, data) => {
				...
				Object.keys(data.headers || {}).forEach((key) => {
					if (shared.isProtoKey(key)) return;
